...
**Companion docs:**
- `docs/tomes/workshops/README.md` (constellation template flow)
- `docs/tomes/workshops/CEREMONY_EVOLUTION.md` (governance of constellation versions)
...
|---|---|---|
| `LatticeMap` | `src/components/guide/achievements/LatticeMap.tsx` | 64-vertex Pascal's-row render; amber-halo lighting per inhabited vertex; ready to accept "lit by witness" predicate |
...
| `AchievementToast` | `src/components/guide/AchievementToast.tsx` | Queued popup; colored border + glow; emoji + title; 5s auto-dismiss; precedent for unlock-celebration UX |
| `addBlade()` / spellweb-blade-bridge | spellweb runtime | Where the witness event would fire |
| `getWitnessCountsByShop()` | `src/lib/shop-witnesses.ts` | Reads the per-shop count from localStorage |
| `PERSONAS_BY_VERTEX` | per the achievements page | Vertex → inhabitants map; mirror exists in spellweb's `vertex-*` nodes |
...
The `*(new)*` nodes need to be added to `spellweb/src/data/nodes.ts` in a follow-up pass — they don't all exist today. The non-new ones already exist and just need to be marked `secret_unlocks_with: shop-<route>` so the renderer knows to hide them initially.
...
```typescript
// Add to spellweb/src/types/graph.ts SpellwebNode interface:
...
1. **Add `hiddenUntilWitness` and friends to SpellwebNode + SpellwebEdge** (`spellweb/src/types/graph.ts`) — non-breaking optional fields.
2. **Mark the existing secret-node candidates** in `spellweb/src/data/nodes.ts` with `hiddenUntilWitness: 'shop-<route>'`.
3. **Add the `*(new)*` concept/spell nodes** flagged in §4 (about 17 new nodes).
4. **Add the unlock edges** in `spellweb/src/data/edges.ts` with `hiddenUntilWitness` matching.
5. **Implement the localStorage layer** at `spellweb/src/lib/witness-unlock.ts` (~80 lines).
