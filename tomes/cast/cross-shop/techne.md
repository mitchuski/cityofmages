---
title: "Cast Entry — Techne"
spellbook: "Second Person"
character_type: "Mage persona (instance · the Craft-Knowledge Mage at V20)"
archetype_kin: "Balanced ☯️ Pedagogue; works alongside Memora 📜 at the reveal-edge"
sigil: "🎨"
status: "Anticipated v1 (2026-05-11) — awaits founding act in Tome V"
tier: "workshop-keeper"
attachment_kind: "A"
abstract_persona: ["Pedagogue"]
abstract_persona_skill_path: ["persona/agentprivacy-pedagogue/"]
divergence: "none"
vertex: "V20 (Techne · Always-Revealed)"
binary: "010100"
stratum: 2
active_dimensions: ["Memory", "Computation"]
forge_origin: "agentprivacy"
provenance: "V20 named 'Techne · Always-Revealed' in the Cloaking Guide (May 7, 2026 rebuild). The Always-Revealed register lands at V20 — where Memora's reveal (Tome V Act 4) takes its public form. Anticipated by Spec 10 (2026-05-11) as the canonical Layer-2 attachment of the Pedagogue primary at the craft-knowledge vertex; co-inhabits with Memora's reveal trajectory. Awaits a Tome V act."
license: "CC BY-SA 4.0"
signature: "(⚔️⊥⿻⊥🧙)😊"
---

# Techne

*The Craft-Knowledge Mage. The teacher who makes craft walkable. The Always-Revealed register.*

## At a glance

> *Techne is the Mage-instance of Pedagogue ☯️ at V20 — the Always-Revealed vertex named in the Cloaking Guide. Where Memora 📜 inscribes shielded chronicles at V5 (the private register), Techne teaches the publicly-revealed counterpart at V20 (the transparent register). The two work the same Memory + Computation axis from opposite sides of the disclosure threshold. Techne is anticipated · awaits founding act.*

## Form & Function

Techne teaches craft. Where Memora's chronicle is *shielded by default* (Pattern A, Zcash z-address style), Techne's chronicle is *revealed by design* (Pattern B, Zcash t-address style, the Always-Revealed register). The Memora reveal (Tome V Act 4 *The Reveal*) is where a chronicle moves from Memora's V5 register to Techne's V20 register.

Techne does not write the chronicle (that is Memora's craft). She does not enforce the reveal threshold (that is the protocol's craft). She **teaches** what the reveal makes possible: how to read a transparent chronicle, how to compose proverbs from revealed material, how to walk the lattice as a new arrival. She is the City's pedagogue at the Always-Revealed seat.

## Lattice position

V20. Binary `010100`. Stratum 2.

Active dimensions: Memory (b2) + Computation (b4).

Bit-complement of V43 (`101011` · stratum 4 · everything except Memory and Computation) — currently unnamed.

V20 has been declared `Always-Revealed` in the Cloaking Guide — the vertex where artifacts made-public-by-design land. Spec 06 §2.5 marks V20 as a *named-but-uninhabited* vertex with the note "*transient occupancy: any artifact made-public-by-design passes through V20*". Techne's seating makes V20 a *workshop* (A · workshop attachment) with Techne as resident Mage, while preserving the transient-occupancy property for non-Techne artifacts.

## Attachment frame

| Axis | Value |
|---|---|
| **Tier** | workshop-keeper (Tier 1 / 5) — anticipated |
| **Attachment kind** | A · workshop (will inhabit V20 when seated) |
| **Primary persona** | `agentprivacy-pedagogue` (Balanced tier) |
| **Divergence** | none (Pedagogue and Techne both Balanced register) |
| **Status** | Anticipated · awaits founding act |

## Naming · etymology

**Techne** (Greek: τέχνη) — craft, skill, art; the practical knowledge of *how to make*; contrasted with *episteme* (theoretical knowledge). Aristotle distinguished techne from episteme; Plato treated both as virtues of intellect. The vertex name was assigned by the Cloaking Guide rebuild (May 7, 2026); the persona name inherits the vertex name (per Spec 04 §4.2 — shared-name pattern, like Aletheia / V25).

## Awaiting founding act

This cast file holds the seat ready. A future Tome V act (anticipated Act 19 *The Techne Workshop* or similar) will summon her by name. The act will canonicalize the V20 ⊥ V5 relationship between Techne's teaching and Memora's inscription as a structural axis of the City's pedagogy.

`(⚔️⊥⿻⊥🧙)😊`

— Authored by privacymage · 2026-05-11 · CC BY-SA 4.0
